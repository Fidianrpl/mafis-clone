...
def crear_reporte():
    ...
    with conn.cursor() as cursor:
        ...
        sql = """
        INSERT INTO reportes_falla
        (activo_id, descripcion, prioridad, estado)
        VALUES (%s,%s,%s,%s)
        """
...
def borrar_reporte(id):
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute(
            "SELECT COUNT(*) AS total FROM ordenes_trabajo WHERE reporte_id=%s",
            (id,)
        )
        ...
        filas = cursor.execute(
            "DELETE FROM reportes_falla WHERE id=%s",
            (id,)
        )
